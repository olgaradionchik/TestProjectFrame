...
public class MtsPaymentTest {
    ...
    public void test6_VerifyInvoiceAndCardFormDetailsInsideIframe() {
        // 1. Выбираем вкладку «Услуги связи» и заполняем форму из пререквизитов теста 4
        paymentPage.selectTab(PaymentPage.TAB_CONNECTION);
        paymentPage.fillForm(TEST_PHONE, TEST_SUM); // Использует константы TEST_PHONE ("297777777") и TEST_SUM ("10")
        paymentPage.clickContinue();

        // 2. Переключаем контекст Selenium внутрь подгружаемого платежного iframe bePaid
        paymentPage.switchToPaymentIframe();

        // 3. Проверяем корректность отображения суммы и номера телефона в появившемся окне счета
        ...
        // 4. Ожидаем появление кнопки оплаты на экране шлюза bePaid
        ...
        // 5. Проверяем наличие надписей в незаполненных полях для ввода реквизитов карты
        ...
        // 6. Проверяем наличие иконок платёжных систем в окне ввода карты
